  ...
  return (
    <SurfBackground category="sunset">
      ...
      <div className="bg-white/10 backdrop-blur-md border-b border-white/20 sticky top-0 z-10">
        <div className="max-w-md mx-auto px-4 py-4">
          ...
          <div className="relative">
            ...
            />
            <svg className="w-5 h-5 text-white/60 absolute right-4 top-1/2 transform -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="max-w-md mx-auto px-4 py-6">
        ...
        {/* Empty State */}
        {favorites.length === 0 ? (
          <div className="bg-white/10 backdrop-blur-md rounded-[15px] p-8 border border-white/20 text-center">
            <div className="text-6xl mb-4">🏄‍♂️</div>
            <h2 className="text-xl font-bold text-white mb-2">No Favorites Yet</h2>
            <p className="text-white/70 mb-6">Start adding your favorite surf spots to keep track of conditions</p>
            <button className="px-6 py-3 bg-white/20 hover:bg-white/30 rounded-full text-white font-medium transition-colors">
              Explore Spots
            </button>
          </div>
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-[15px] p-8 border border-white/20 text-center">
            ...
            <h2 className="text-lg font-bold text-white mb-2">No matches found</h2>
          ...
          <div className="space-y-4">
            {filteredSpots.map((spot) => (
              <div key={spot.id} className="bg-white/10 backdrop-blur-md rounded-[15px] p-4 border border-white/20">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    ...
                    <p className="text-white/70 text-sm">{spot.region}, {spot.state}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2">
                      <span className="text-2xl">{getQualityEmoji(spot.conditionsRating)}</span>
                      <div className={`px-3 py-1 rounded-full text-sm font-medium ${getConditionsBadgeColor(spot.conditionsRating)} text-white`}>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="bg-white/10 rounded-lg p-3">
                    ...
                    <p className="text-white font-bold text-lg">{spot.currentConditions.swellHeight}ft</p>
                  </div>
                  <div className="bg-white/10 rounded-lg p-3">
                    <p className="text-white/70 text-xs uppercase tracking-wide">Period</p>
                    <p className="text-white font-bold text-lg">{spot.currentConditions.swellPeriod}s</p>
                  </div>
                  <div className="bg-white/10 rounded-lg p-3">
                    <p className="text-white/70 text-xs uppercase tracking-wide">Wind</p>
                    <p className="text-white font-bold text-lg">{spot.currentConditions.windSpeed}mph {spot.currentConditions.windDirection}</p>
                  </div>
                  <div className="bg-white/10 rounded-lg p-3">
                    <p className="text-white/70 text-xs uppercase tracking-wide">Water Temp</p>
                    <p className="text-white font-bold text-lg">{spot.currentConditions.waterTemp}°F</p>
                  </div>
                </div>
